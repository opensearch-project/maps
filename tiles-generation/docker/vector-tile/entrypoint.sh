...
usage() {
    ...
    echo -e "AWS_ACCESS_KEY_ID     \tSpecify the AWS access key id used used for s3 copy"
    echo -e "AWS_SECRET_ACCESS_KEY \tSpecify the AWS AWS secret access key used for s3 copy"
    echo -e "JAVA_TOOL_OPTIONS     \tSpecify the JVM config, for example:'-Xmx1g'"
    echo -e "STORAGE               \t(Needed for planet tile generation)Specify node location cache in memory by 'ram' or in a temporary memory-mapped file by 'mmap'"
    echo ""
    echo "--------------------------------------------------------------------------"
}
...
validateParameter() {
    if [ -z "$AREA" ] || [ -z "$TILE_S3_BUCKET" ] || [ -z "$AWS_ACCESS_KEY_ID" ] || [ -z "$AWS_SECRET_ACCESS_KEY" ]; then
      echo "You must specify '-e AREA', '-e TILE_S3_BUCKET', '-e AWS_ACCESS_KEY_ID', '-e AWS_SECRET_ACCESS_KEY' in container environment variables"
      usage
      exit 1
    fi
}
